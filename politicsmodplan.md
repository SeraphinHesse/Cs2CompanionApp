...
- **Systems:** EU theme = proportional, 4–7 parties, coalitions, 3-year terms. NA theme = FPTP, two dominant parties + weak third parties, directly elected mayor, 4-year terms. Theme follows the map theme choice; overridable in settings.
...
**`src/Agora.Core/` — pure C#, no game references, netstandard2.1**
...
**`src/Agora.Mod/` — game glue, references `Game.dll` et al, netstandard2.1**
...
### M0 · Bootstrap ⟐
Tasks: repo scaffold; router + folder CLAUDE.mds; skills (§9); agent definitions; two-assembly build; `Agora.Core` determinism kernel + test suite; loadable mod with options page and per-day heartbeat; **UI pipeline smoke test** (one `GetterValueBinding` rendered by a React panel); Scout report #1; `refsrc/` decompiled reference tree; graphify port (Serph).
...
*Rationale for the UI smoke test: the Gameface pipeline was the largest unknown in the stack, and under the original plan nothing touched it until M2's gate depended on it. Half a day here removes that risk.*
...
8. ⟐ **Node version drift.** The CS2 UI templates target Node 20.11; this machine has Node 24. Pin with nvm if webpack misbehaves.
